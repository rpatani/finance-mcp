#!/usr/bin/env bash
# End-to-end smoke test for a running finance-mcp HTTP server.
#   ./deploy/smoke-test.sh [http_base] [metrics_base]
# Exercises /healthz, /readyz, an MCP session (initialize -> initialized ->
# tools/list -> tools/call get_crypto_price), and asserts the metrics counter
...
EXPECTED="get_company_overview get_crypto_market get_crypto_price get_fx_rate get_stock_quote "
[ "$TOOLS" = "$EXPECTED" ] && pass "5 tools listed" || fail "unexpected tools: [$TOOLS]"

echo "== tools/call get_crypto_price =="
CALL=$(curl -sf -L -H "Content-Type: application/json" -H "Accept: ${ACCEPT}" -H "mcp-session-id: ${SESSION}" \
  -X POST "$MCP_URL" -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"get_crypto_price","arguments":{"coin_id":"bitcoin"}}}' | sse_data)
[ "$(printf '%s' "$CALL" | jq -r '.result.isError')" = "false" ] && pass "get_crypto_price ok" || fail "call errored: $CALL"

echo "== metrics =="
if curl -sf "${METRICS_BASE}/metrics" | grep '^mcp_tool_calls_total{' | grep 'tool="get_crypto_price"' | grep -q 'status="success"'; then
  pass "mcp_tool_calls_total incremented for get_crypto_price"
else
  fail "success metric not found for get_crypto_price"
fi
